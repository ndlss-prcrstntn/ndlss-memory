...
- [ ] T001 Выполнить MCP-контекстный проход (`search_docs` -> `semantic_search` -> чтение файлов) и зафиксировать источники для задач multi-module
...
- Перед правками в multi-module задачах: MCP контекст -> локальная верификация (`rg`, чтение файлов)
...
- При устаревшем MCP-индексе добавляйте отдельную задачу на его обновление до активной реализации
