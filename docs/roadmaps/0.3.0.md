...
- [x] Автоматическое создание коллекции в Qdrant при старте стека (без первого ingestion-upsert)
- [x] Автоматический запуск первичной индексации при первом запуске (one-time bootstrap)
...
- [x] Режим `INDEX_MODE=watch` (непрерывная индексация изменений)
...
- [x] Лимит глубины обхода директории
- [x] Лимит количества файлов на запуск

## 4. Пайплайн чанкинга и эмбеддингов

- [x] Отдельная коллекция для md-документов (без смешивания с кодовыми чанками)
- [ ] Hybrid search (BM25 + vector) только по md-коллекции
- [ ] Reranking поверх hybrid-выдачи только по md-коллекции
- [ ] Расширенные метаданные чанков:
- [ ] `line_start`
- [ ] `line_end`
- [x] стабильный `chunk_index` в публичном контракте metadata
- [ ] Symbol metadata:
- [ ] список функций
- [ ] список классов
- [ ] список импортов
...
- [x] Интеграционные/контрактные тесты watch-mode
...
- [x] watch-mode покрыт интеграционными тестами
- [ ] новые MCP tools покрыты contract-тестами и примерами запросов в docs
- [x] опубликован релиз `0.3.0` с обновленным changelog и roadmap snapshot
